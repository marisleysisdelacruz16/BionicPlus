	...
	Course.find( {}, (err, courses) => {
		if (err) {
			...
			// this creates a link to the /updateCourseView endpoint
			    ...
			    res.write('</li>');
			    // this creates a link to the /deleteCourseWarning endpoint
			    res.write(" <a href=\"/deleteCourseWarning?name=" + course.name + "\">[Delete Course]</a>");
			    res.write('</li>');
		...
		if (err) {
			...
			// Writes out all courses in the database
				...
				// this creates a link to the /updateCourseView endpoint, which brings the user to the related html page
				 res.write(" <a href=\"/updateCourseView?name=" + course.name +  "&ID="+course._id+"\">[EditCourse]</a>");
				    res.write('</li>');
				    // this creates a link to the /deleteCourseWarning endpoint
				    res.write(" <a href=\"/deleteCourseWarning?name=" + course.name + "\">[Delete Course]</a>");
				    res.write('</li>');
				// this creates a link to the /addClassView endpoint, which brings the user to the related html page
				res.write(" <a href=\"/addClassView?name=" + course.name + "&ID="+course._id+"\">[Add Class]</a>");
				    res.write('</li>');
				res.write('</ul>');
...
//Set to req.body.description instead of req.query.description since we'll be doing this through an html form.
    var action = {'$set': {description: req.body.description,level: req.body.level, department: req.body.department}}
